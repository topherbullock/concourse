...
var _ = Describe("Worker", func() {
	...
	Describe("FindCreatedContainerByHandle", func() {
		...
		Context("when the gardenClient returns a container and no error", func() {
			BeforeEach(func() {
				fakeGardenContainer.HandleReturns("provider-handle")
			...
			It("returns the container", func() {
				...
				Expect(found).To(BeTrue())
				Expect(foundContainer.Handle()).To(Equal(fakeGardenContainer.Handle()))
			})
			...
			Context("when the user property is present", func() {
				...
				BeforeEach(func() {
					...
					fakeGardenContainer.PropertiesReturns(garden.Properties{"user": "maverick"}, nil)
				})
				...
				Describe("Run", func() {
					It("calls Run() on the garden container and injects the user", func() {
						Expect(fakeGardenContainer.RunCallCount()).To(Equal(1))
						spec, io := fakeGardenContainer.RunArgsForCall(0)
			...
			Context("when the user property is not present", func() {
				...
				BeforeEach(func() {
					...
					fakeGardenContainer.PropertiesReturns(garden.Properties{"user": ""}, nil)
				})
